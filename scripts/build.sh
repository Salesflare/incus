...
cp "$GOPATH"/src/github.com/Salesflare/incus/scripts/test_config/config.yml "$GOPATH"/bin/
...
go vet ./...  && \
go test -v $BUILDR && \
tar cf "$GOPATH"/incus.tar -C "$GOPATH" bin/incus -C "$GOPATH"/src/github.com/Salesflare/incus scripts/ appspec.yml
